...
class Dash {
  constructor() {
    this.state = { q: '', sel: null, tier: 'logo', view: null, vw: 0, vh: 0, autoFormat: null, focusCat: null, focusSub: null, sheetOpen: false };
    this.v = { x: 0, y: 0, k: 1 };
    this.broken = {};
    this.dragged = false;
  ...
  select(m) { if (this.dragged) { this.dragged = false; return; } this.setState({ sel: m }); }
  ...
  bind() {
    ...
    document.addEventListener('pointerdown', e => {
      ...
      if (e.target.closest('[data-detail-panel]') || e.target.closest('button') || e.target.closest('input') || e.target.closest('a')) return;
      this.setState({ sel: null });
    });
    window.addEventListener('keydown', e => { if (e.key === 'Escape' && this.state.sel) this.setState({ sel: null }); });
  ...
  renderMap(tier, q) {
    ...
    const parts = l.clusters.map(c => {
      ...
      return '<div style="position:absolute; left:' + c.x + 'px; top:' + c.y + 'px; width:' + c.w + 'px; height:' + c.h + 'px; background:' + c.bg + '; border:1.5px solid ' + c.border + '; border-radius:14px;">'
        ...
        + '<span style="font-family:\'Barlow Condensed\',sans-serif; font-weight:700; font-size:' + Math.round(c.headFont * 0.5) + 'px; color:' + c.ink + '; opacity:.45; flex:none;">' + c.count + '</span></div>'
  ...
  renderList(cats, q) {
    ...
    const sections = cats.map(c => {
      ...
      return '<section data-sec="' + c.slug + '" style="margin:0 0 10px;">'
        ...
        + '<span style="font-family:\'Barlow Condensed\',sans-serif; font-weight:700; font-size:15px; color:' + roleColor(T, 'ink') + '; opacity:.5;">' + n + '</span></div>'
        + groups.join('') + '</section>';
    }).join('');
